...
- **Feedback:** **separate glass toasts** (bottom) for transient confirmations, **undo-on-delete** ("Deleted · Undo"), and errors — reachable, near the action; the **Dynamic Island stays status-only** (downloads/processing). Form validation inline. (Fills the gap left by dropping `time-undo`.)
- **Settings:** a single scrollable **grouped page** (iOS-style) — titled sections (Downloads · Network · Reading · Appearance · Advanced), labeled rows + controls, **solid content layer**. No nested settings-sidebar (would clash with the app nav).
- **Onboarding:** a **single light glass welcome** on first launch (name · "downloads go to *X* — change in Settings" · Get started), then dismissed forever. No setup wizard — every setting already has a sane default.
- **Drag-drop import:** **non-goal (YAGNI)** — San Citro is an AA downloader, not a library manager; the library is your AA downloads only. (Drag-to-*read* via the multi-format reader is a logged future option, not built now.)
